...
public class CreateTaskFragment extends Fragment {
    ...
    private View layoutRecurring;
    private TextView tvSelectedDate, tvSelectedTime, tvSelectedEndDate;
    private Button btnPickDate, btnPickTime, btnPickEndDate, btnSave;
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        if (getArguments() != null) {
            editingTaskId = getArguments().getString(ARG_TASK_ID, null);
        }
    ...
    private void initViews(View root) {
        etName = root.findViewById(R.id.etTaskName);
        etDesc = root.findViewById(R.id.etTaskDescription);
        etInterval = root.findViewById(R.id.etInterval);
        spinnerCategory = root.findViewById(R.id.spinnerCategory);
        spinnerUnit = root.findViewById(R.id.spinnerUnit);
        rgDifficulty = root.findViewById(R.id.rgDifficulty);
        rgImportance = root.findViewById(R.id.rgImportance);
        switchRecurring = root.findViewById(R.id.switchRecurring);
        layoutRecurring = root.findViewById(R.id.layoutRecurringOptions);
        ...
        tvSelectedEndDate = root.findViewById(R.id.tvSelectedEndDate);
        btnPickEndDate = root.findViewById(R.id.btnPickEndDate);
        btnSave = root.findViewById(R.id.btnSaveTask);
    }
    ...
    private void loadCategories() {
        categoryService.getAllCategories(categories -> {
            ...
            if (categories != null) loadedCategories.addAll(categories);

            List<String> names = new ArrayList<>();
            for (Category c : loadedCategories) names.add(c.getName() != null ? c.getName() : "Unnamed");
            ...
            // tek kad se kategorije ucitaju, onda ucitaj task za edit (da selection radi)
            if (editingTaskId != null) loadTaskForEdit(editingTaskId);
        });
    }
    ...
    private void loadTaskForEdit(String taskId) {
        taskService.getTaskById(taskId, task -> {
            if (task == null) {
                Toast.makeText(requireContext(), "Task not found (or no permission).", Toast.LENGTH_SHORT).show();
    ...
    private void saveNewTask() {
        ...
        taskService.addTask(newTask, new TaskService.OnTaskActionEventListener() {
            ...
            public void onSuccess(String message) {
                ...
                NavHostFragment.findNavController(CreateTaskFragment.this).popBackStack();
            }
    ...
    private Task buildTaskFromForm(Task baseTask) {
        ...
        if (switchRecurring.isChecked()) {
            ...
            try {
                ...
                etInterval.setError("Must be positive");
                return null;
            }
